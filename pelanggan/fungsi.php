...
function tambah_bukti($data)
{
    ...
    if ($ukuran <= 2048000) {
        move_uploaded_file($lokasi_file, 'pelanggan/img/' . $nama_file);
        $query = "INSERT INTO bukti_pembayaran (id_bukti,id_detail,foto_bukti,tanggal_pesan) VALUES ('$id_bukti','$id_detail','$image',current_timestamp())";

        $hasil = mysqli_query($conn, $query);
        if ($hasil) {
            echo "<script> alert ('Proses Tambah bukti pembayaran Berhasil'); window.location='blog-details.php';</script>";
        } else {
            echo "<script> alert ('Proses Tambah bukti pembayaran Gagal'); history.back();</script>";
        }
    } else {
        echo "<script> alert ('Ukuran gambar $nama_file lebih dari 2 MB, Tidak Bisa Tambah Data'); window.location='checkout.php'</script>";
    }
...
function ubah_bukti($data)
{
    ...
    $query = "UPDATE bukti_pembayaran SET id_detail='$id_detail', foto_bukti = '$image', tanggal_pesan = '$tanggal_pesan' WHERE id_bukti='$id_bukti';";
     $hasil = mysqli_query($conn, $query);
     if ($hasil) {
     echo "<script> alert ('Proses Ubah Bukti Pembayaran Berhasil'); window.location='checkout.php';</script>";
     } 
     else {
...
// menampilkan total pembayaran menggunakan function
function menampilkan_total_pembayaran($nama_pelanggan)
{
    //memanggil variabel global conn
    global $conn;
    $query = "SELECT cek_total_pembayaran_pelanggan('$nama_pelanggan') AS total";
    $result = mysqli_query($conn, $query);
