  ...
  ?>
    ...
    <section class="shoping-cart spad">
        <div class="container">
                    ...
                    <div class="checkout__order">
                        ...
                        <div class="shoping__cart__table">
                            <table>
                                <thead>
                                    <tr>
                                        <th class="shoping__product">Riwayat Pesanan</th>
                                        <th>Tanggal</th>
                                        <th>Harga</th>
                                        <th>Jumlah</th>
                                        <th>Total</th>
            ...
            $nama_pelanggan = $_SESSION['nama_pelanggan'];
            $total = menampilkan_total_pembayaran($nama_pelanggan);
            ?>
                        <?php
                $id=$_SESSION['nama_pelanggan'];
                $data = mysqli_query($conn, "SELECT pelanggan.*, menu.*,orderan.*, detail_orderan.*, bukti_pembayaran.*, SUM(total_harga) AS harga_total, SUM(ongkir) AS total_ongkir,SUM((menu.Harga*detail_orderan.jumlah_orderan) + ongkir) AS pembayaran_total FROM pelanggan JOIN orderan USING (id_pelanggan) JOIN menu USING (id_menu) 
                JOIN detail_orderan USING (id_orderan) JOIN bukti_pembayaran USING (id_detail) WHERE nama_pelanggan = '$id'");   
                while ($row = mysqli_fetch_array($data)) {
                ?>
                        <div class="checkout__order__subtotal">Total Harga<span>
                                Rp.<?php echo number_format($row['harga_total']) ?></span></div>
                        <div class="checkout__order__subtotal">Total Ongkir <span>
                                Rp.<?php echo number_format($row['total_ongkir']) ?></span>
                        </div>
                        <div class="checkout__order__total">Total Pembayaran
                            <span> Rp.<?php echo number_format($total['total']) ?></span>
                        </div>
                ...
                ?>
                    ...
                    <div class="mt-3">
                        ...
                        <a href="riwayat-orderan.php" class="primary-btn cart-btn cart-btn-left">
                            Cek Keranjang</a>
                    </div>
                </div>
            </div>
